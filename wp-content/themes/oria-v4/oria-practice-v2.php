...
<nav class="spine" aria-label="<?php esc_attr_e( 'Page sections', 'oria' ); ?>">
	<div class="wrap spine__row">
		...
		if ( $oria_events ) {
			// Above the listings in v4, so first in the menu too.
			$oria_floors[] = array( '#events', __( "What's on", 'oria' ) );
		}
		/* translators: %s: number of listings */
		$oria_floors[] = array( '#browse', sprintf( __( 'Listings (%s)', 'oria' ), number_format_i18n( count( $oria_ids ) ) ) );
...
?>
<!-- Floor 1 — Decide: text left, the category's own picture right -->
...
<div class="xc-hero__text">
	...
	<div class="cathero">
		...
		<?php if ( ! $oria_facet && $oria_answer['sentences'] ) : ?>
			<?php // The answer sentences, promoted from "About these results" to the canvas's "Is this for me?" card. ?>
			<div class="xc-forme">
				<h2 class="h3 xc-forme__title"><?php esc_html_e( 'Is this for me?', 'oria' ); ?></h2>
		...
		<details class="catabout">
			...
			<div class="catabout__body">
				<?php if ( $oria_facet ) : ?>
					...
					<?php endif; ?>
				<?php endif; // The answer sentences are in the "Is this for me?" card above. ?>
				<p class="hint">
...
<?php if ( '' !== $oria_hero_img ) : ?>
	<?php // Decoration: the empty alt keeps screen readers on the heading. ?>
	<figure class="xc-hero__pic"><img src="<?php echo esc_url( $oria_hero_img ); ?>" alt="" width="720" height="760" fetchpriority="high" decoding="async"></figure>
<?php endif; ?>
</section>

<?php
/*
 * The short guide before the list, from the canvas: ways to begin, a
 * first-timer note, the editors' shortlist and what's on. Base view only
 * -- a facet or suburb page has already made its decision and goes
 * straight to its listings.
 */
$oria_base = $oria_term && ! $oria_facet && ! $oria_area;

// Ways to begin: the three largest of the category's own choices (the
// same rows as the quick filters), each a real filtered view.
$oria_ways = array();
if ( $oria_base ) {
	$oria_ways = $oria_rows;
	usort( $oria_ways, static fn( array $a, array $b ): int => (int) $b['count'] <=> (int) $a['count'] );
	$oria_ways = array_slice( array_values( array_filter( $oria_ways, static fn( array $r ): bool => (int) $r['count'] >= 3 ) ), 0, 3 );
	if ( count( $oria_ways ) < 3 ) {
		$oria_ways = array();
	}
}

// The first-timer note: the introduction's opening paragraph. The rest of
// the introduction stays in the guide below, so nothing is printed twice.
$oria_intro_html = ( is_string( $oria_intro ) && '' !== trim( $oria_intro ) && $oria_term )
	...
	: '';
$oria_intro_first = '';
$oria_intro_rest  = $oria_intro_html;
if ( $oria_base && '' !== $oria_intro_html ) {
	$oria_cut = stripos( $oria_intro_html, '</p>' );
	if ( false !== $oria_cut ) {
		$oria_intro_first = substr( $oria_intro_html, 0, $oria_cut + 4 );
		$oria_intro_rest  = trim( substr( $oria_intro_html, $oria_cut + 4 ) );
	}
}
?>

<?php if ( $oria_ways ) : ?>
	<section class="wrap xc-ways" aria-labelledby="xc-ways-title">
		<h2 class="h2 xc-ways__title" id="xc-ways-title"><?php esc_html_e( 'Ways to begin', 'oria' ); ?></h2>
		<div class="xc-ways__grid">
			<?php
			foreach ( $oria_ways as $oria_w ) :
				/*
				 * The row's own facet picture where the theme ships one. Rows are
				 * keyed "page:yin" while the pictures are named "yin-yoga", so try
				 * the key, the label, the key with the category, then the
				 * category's own picture.
				 */
				$oria_wkey = function_exists( '\Oria\Core\Intents\row_key' ) ? \Oria\Core\Intents\row_key( $oria_w ) : '';
				$oria_wval = '' !== $oria_wkey ? (string) substr( $oria_wkey, (int) strpos( $oria_wkey, ':' ) + 1 ) : '';
				$oria_wimg = '';
				foreach ( array( $oria_wval, sanitize_title( (string) $oria_w['label'] ), $oria_wval . '-' . $oria_term->slug, $oria_term->slug ) as $oria_wtry ) {
					$oria_wimg = '' !== $oria_wtry ? \Oria\Theme\facet_image( $oria_wtry ) : '';
					if ( '' !== $oria_wimg ) {
						break;
					}
				}
				?>
				<a class="xc-way" href="<?php echo esc_url( $oria_row_url( $oria_w ) ); ?>" data-oria-event="category_quick_filter_select">
					<span class="xc-way__pic<?php echo '' === $oria_wimg ? ' xc-way__pic--bare' : ''; ?>">
						<?php if ( '' !== $oria_wimg ) : ?>
							<img src="<?php echo esc_url( $oria_wimg ); ?>" alt="" loading="lazy" decoding="async" width="480" height="360">
						<?php endif; ?>
					</span>
					<span class="micro xc-way__n">
						<?php
						/* translators: %s: number of places */
						printf( esc_html( _n( '%s place', '%s places', (int) $oria_w['count'], 'oria' ) ), esc_html( number_format_i18n( (int) $oria_w['count'] ) ) );
						?>
					</span>
					<span class="h3 xc-way__label"><?php echo esc_html( (string) $oria_w['label'] ); ?></span>
				</a>
			<?php endforeach; ?>
		</div>
	</section>
...
<?php if ( '' !== $oria_intro_first ) : ?>
	<section class="wrap xc-first" aria-labelledby="xc-first-title">
		<div class="xc-first__band on-deep">
			<div class="xc-first__head">
				<p class="micro"><?php esc_html_e( 'Before you go', 'oria' ); ?></p>
				<h2 class="h2" id="xc-first-title">
					<?php
					/* translators: 1: category, lower case, 2: city */
					printf( esc_html__( 'How %1$s is taught in %2$s', 'oria' ), esc_html( strtolower( $oria_pname ) ), esc_html( $oria_cname ) );
					?>
				</h2>
			</div>
			<div class="xc-first__body">
				<div class="prose"><?php echo wp_kses_post( $oria_intro_first ); ?></div>
				<?php if ( '' !== $oria_intro_rest ) : ?>
					<a class="xc-first__more" href="#read"><?php esc_html_e( 'Read the rest of the guide', 'oria' ); ?> <span aria-hidden="true">&darr;</span></a>
				<?php endif; ?>
			</div>
		</div>
	</section>
...
<?php
// The editors' shortlist and what's on, moved up from under the list.
if ( $oria_bo_lead ) {
	echo '<div class="wrap xc-shelf">';
...
<section class="wrap section section--top-flush floor" id="browse">
	...
	?>
	<?php // The Best Of shelf sits above the listings in v4. ?>
...
<section class="wrap section floor" id="read">
	...
	<?php elseif ( '' !== $oria_intro_rest ) : ?>
		<?php // Its opening paragraph is the "Before you go" band above the listings. ?>
		<h2 class="h3" style="margin-bottom:1rem">
			<?php
			if ( '' !== $oria_intro_first ) {
				esc_html_e( 'More before you book', 'oria' );
			} else {
				printf( esc_html__( 'How %1$s is taught in %2$s', 'oria' ), esc_html( strtolower( $oria_pname ) ), esc_html( $oria_cname ) );
			}
			?>
		</h2>
		<div class="prose prose--intro"><?php echo wp_kses_post( $oria_intro_rest ); ?></div>
	<?php endif; ?>
...
<?php
// What's on sits above the listings in v4, as "Coming up".
?>
