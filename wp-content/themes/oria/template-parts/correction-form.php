 ...
 *
 * $args: id (listing post id), bare (bool -- drop the <details> fold when
 * the caller has already provided one, such as the claim dialog)
 */
...
/*
 * Inside the "Suggest an update" dialog the fold would be a second lid on
 * a box already open, so the caller asks for the body alone. The form,
 * nonce, honeypot and handler are the same either way -- only the wrapper
 * differs.
 */
$oria_cbare = ! empty( $args['bare'] );
?>
		...
		<div class="ocorr__done notice" role="status" data-oria-event="correction_submitted">
			...
			<?php if ( $oria_cowner && $oria_cclaim ) : ?>
				<p class="ocorr__offer">
					...
					<a class="ocorr__offerlink" href="#claim"><?php esc_html_e( 'Claim this profile', 'oria' ); ?></a>
				</p>
			<?php endif; ?>
		</div>
	<?php else : ?>
		...
		<details class="ocorr__fold"<?php echo 'error' === $oria_cstate ? ' open' : ''; ?>>
			<summary class="ocorr__open"><?php esc_html_e( 'Suggest an edit', 'oria' ); ?></summary>
		<?php endif; ?>
